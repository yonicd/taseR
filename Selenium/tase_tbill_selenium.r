...
tase.tbill.daily=ddply(df.in,.(Name),.fun = function(df){
  #set url
  url=paste0("http://www.tase.co.il/Eng/General/BONDS/Pages/BondsHistoryData.aspx?",
             "subDataType=",5,
             "&shareID=",df$secID,
             "&bondType=")
  ...
  con$navigate(url)
  #enter ui variables
  prefix="ctl00_SPWebPartManager1_g_ed8af170_7f0e_440a_85fe_19d9352a2a86_ctl00_HistoryData1_"
...
tase.tbill.intraday=ddply(df.in,.(Name),.fun = function(df){
  #set url
  url=paste0("http://www.tase.co.il/Eng/General/BONDS/Pages/BondsHistoryData.aspx?",
             "subDataType=",5,
             "&shareID=",df$secID,
  ...
  out=htmlParse(con$getPageSource(),asText = T)
  dataNode =getNodeSet(out,("//table[contains(@id,'gridHistoryData_DataGrid1')]"))
  metaNode =xpathSApply(out,('//*[contains(concat( " ", @class, " " ), concat( " ", "tabTitleGridText", " " ))]'),xmlValue)

  #organise into data.frame
  tase.out=
    readHTMLTable(dataNode[[1]],header = T,stringsAsFactors = F)%>%
...
tase.tbill.otc=ddply(df.in,.(Name),.fun = function(df){
  url=paste0("http://www.tase.co.il/Eng/General/BONDS/Pages/BondsHistoryData.aspx?",
             "subDataType=",5,
             "&shareID=",df$secID,
             "&bondType=")
